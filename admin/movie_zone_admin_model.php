<?php
/*-------------------------------------------------------------------------------------------------
@Module: movie_zone_admin_model.php
...
class MovieZoneAdminModel {
	...
	*/
	...
	Selects all movies from the database
	*/
	public function selectMoviesInventory() {
		//reset the error first
		$this->error = null;
		$this->dbAdapter->dbOpen();
		$result = $this->dbAdapter->moviesInventory();
		$this->dbAdapter->dbClose();
		if ($result == null){
			$this->error = $this->dbAdapter->lastError();
		}
	...
	*/
	public function updateMovie($moviedata) {
		...
		*/
		...
		if ($result == null) {
			$moviedata['director_id'] = $this->dbAdapter->directorAdd($director);
		} else {
			$moviedata['director_id'] = $result[0]['director_id'];
		}
		...
		*/
		...
		$result = $this->dbAdapter->studioSelect($studio);
		if ($result == null) {
			$moviedata['studio_id'] = $this->dbAdapter->studioAdd($studio);
		} else {
			$moviedata['studio_id'] = $result[0]['studio_id'];
		}
		...
		*/
		...
		if ($thumbpath_file === 'default.jpg') {
				$moviedata['thumbpath'] = $moviedata['photo'];
		}
	...
	*/
	...
	/*
	Delete members with given ids
	*/
	public function deleteMemberById($memberids) {
		$this->error = null; //reset the error first
		$this->dbAdapter->dbOpen();
		//then delete members from database
